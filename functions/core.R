...
# ASR: download and clean the ASR data from popdata. Input is the year, output is the dataset. This function exploits the popdata package so make sure you login with pd_login.

# MYSR: similar to ASR but for MYSR data.

# POC: function to get the total poc for aggregations at different level. The input is the raw data generated by ASR or MYSR function, the group level and year. The group level are the variables names of the raw data, recommend using 'region_d' for the regions, 'asylumCountry' for CoA or 'originCountry' for COO.
...
Region_prep <- function(Region, Year, Mid_End){
  ...
  file_path <- file.path("data-raw\\", paste0(Region,"_",Year ,"_clean_data.Rdata"))
